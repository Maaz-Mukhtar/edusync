...
// PUT /api/teacher/online-tests/[testId]/questions/reorder - Reorder questions
  ...
  try {
    ...
    if (onlineTest.status === "PUBLISHED") {
      return NextResponse.json(
        { error: "Cannot reorder questions in a published test" },
        { status: 400 }
      );
    }
    ...
    // Verify all questions belong to this test's assessment
    ...
    if (existingQuestions.length !== questionIds.length) {
      return NextResponse.json(
        { error: "Some questions do not belong to this test" },
        { status: 400 }
      );
    }
